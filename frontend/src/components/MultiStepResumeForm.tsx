...
export default function MultiStepResumeForm() {
  ...
  const [maxVisitedStep, setMaxVisitedStep] = useState(0); // Kept from original
  const [errors, setErrors] = useState<Record<string, string>>({}); // Kept from original
  const fileInputRef = useRef<HTMLInputElement | null>(null); // Kept from original
  const [rawJson, setRawJson] = useState<any | null>(null); // Kept from original
  const [refinedJson, setRefinedJson] = useState<BackendJson | null>(null); // Kept from original, assuming BackendJson is `any` for this context

  const [form, setForm] = useState<FormState>(() => {
    // Try to load from sessionStorage on mount
    if (typeof window !== 'undefined') {
      const saved = sessionStorage.getItem('onboardingFormState');
      if (saved) {
        try {
    ...
    return {
      profile: {
        ...
        phone: "",
        location: "",
        summary: "", // Added back summary
        github: "", // Added back github
        linkedin: "",
        portfolio: "",
  ...
  useEffect(() => {
    sessionStorage.setItem('onboardingFormState', JSON.stringify(form));
  }, [form]);
  ...
  >(section: K, index: number, field: keyof FormState[K][number]) {
    return (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
      ...
      // Special validation for GPA field: only allow positive numbers and decimal point
      if (section === "education" && field === "gpa") {
  ...
  function validateStep(stepIndex: number): Record<string, string> {
    ...
    if (stepIndex === 0) {
      ...
      } else if (!/^[a-zA-Z\s]+$/.test(form.profile.name.trim())) {
      ...
      if (form.certificates.issueDate && form.certificates.expiryDate) {
        const issueDate = new Date(form.certificates.issueDate);
        const expiryDate = new Date(form.certificates.expiryDate);
        if (issueDate > expiryDate) {
          errs.certificateDate = "Issue date cannot be after expiry date";
        }
      }
    } else if (stepIndex === 3) {
      ...
      // Validate start year and end year for all education entries
      form.education.forEach((edu, index) => {
        if (edu.startYear && edu.endYear) {
          const startYear = parseInt(edu.startYear);
          const endYear = parseInt(edu.endYear);
          if (!isNaN(startYear) && !isNaN(endYear) && startYear > endYear) {
            errs[`educationYear${index}`] = `Education ${index + 1}: Start year cannot be after end year`;
          }
        }
      });
    } else if (stepIndex === 5) {
  ...
  async function onSubmit() {
    ...
    if (Object.keys(v).length === 0) {
      try {
        // Save profile to backend before redirecting
        const supabase = createClient();
        const { data: { user } } = await supabase.auth.getUser();
        
        if (user) {
          // Extract previous roles from experience
          const previousRoles = form.experience
            .map(exp => exp.role)
          ...
          const profileData = {
            ...
            certificates: form.certificates ? [form.certificates] : [],
        ...
        // Continue to recommendations even if save fails
      }
      
      // Check if we have cached recommendations for these skills
      const cachedRecs = sessionStorage.getItem('recommendations');
      const cachedSkills = sessionStorage.getItem('userSkills');

      if (cachedRecs && cachedSkills) {
        try {
          const cached = JSON.parse(cachedSkills);
          // Check if cached skills match current skills
          const skillsMatch =
            form.skills.length === cached.length &&
            form.skills.every((skill: string) => cached.includes(skill));

          if (skillsMatch) {
            // Use cached recommendations - navigate without skills param
            window.location.href = '/recommendations';
            return;
          }
        } catch (e) {
          console.error('Error checking cached recommendations:', e);
        }
      }

      // No cache or skills don't match - pass skills to trigger new fetch
        ...
        <div className="w-full flex items-center justify-center mb-4">
          <ResumeAutofillButton
            onAutofill={(data) => {
              ...
              // Clear ALL session state when new resume is uploaded for complete reset
              sessionStorage.removeItem('onboardingFormState');
              sessionStorage.removeItem('userSkills');
              sessionStorage.removeItem('recommendations');
              sessionStorage.removeItem('learningResourcesSkills');
              sessionStorage.removeItem('learningPreferences');
              sessionStorage.removeItem('learningResources');
              sessionStorage.removeItem('currentLearningSkills');

              // Clear learned skills from localStorage
              localStorage.removeItem('learnedSkills');
          ...
          {currentStep === 2 && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {errors.certificateDate && (
                <div className="md:col-span-2">
                  <p className="text-xs text-red-600">{errors.certificateDate}</p>
                </div>
              )}
            </div>
          )}
