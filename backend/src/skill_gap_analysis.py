...
def analyze_skill_gap(role_name: str, user_skills: List[str]) -> Dict[str, Any]:
    ...
    # Count how many required skills were actually matched (not how many user skills matched)
    num_matched_required_skills = len(required_skills) - len(comparison_result["required_skills"])
    comparison_result["total_existing_skills"] = num_matched_required_skills
    comparison_result["total_missing_skills"] = len(comparison_result["required_skills"])
    
    # Calculate completion percentage based on required skills coverage
    # Formula: (Matched Required Skills / Total Required Skills) * 100
    if len(required_skills) > 0:
        completion_percentage = (num_matched_required_skills / len(required_skills)) * 100
